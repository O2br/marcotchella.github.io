(function(ext) {
	...
	var blocks = {
		...
		el: [
				[" ", "????s?? a??ste?? %d.motorvalue right %d.motorvalue","runBot", 100, 100],
				[" ", "???se t?? ????t??a%d.motorPort speed %d.motorvalue","runMotor", "M1", 0],
				[" ", "???se t?? se?ß?????t??a %d.port %d.slot angle %d.servovalue","runServo", "Port1","Slot1", 90],
				[" ", "???se t? led %d.lport %d.slot %d.index red%d.value green%d.value blue%d.value","runLed","on board","Slot1","all",0,0,0],
				[" ", "pa??e t??? st? ??ta %d.note beat %d.beats","runBuzzer", "C4", "Half"],
				[" ", "de??e p??s?p? %d.port x:%n y:%n characters:%s","showCharacters", "Port1", 0,0,"Hello"],
				[" ", "de??e ??a %d.port hour:%n %m.points min:%n","showTime", "Port1", 10,":",20],
				[" ", "de??e ????af?? %d.port x:%n y:%n draw:%m.drawFace","showDraw", "Port1", 0,0,"        "],
				["-"],
				[" ", "???se t?? ????? 7 tµ?µ?t?? %d.port number %n","runSevseg", "Port1", 100],
				[" ", "???se t?? a?s??t??a f?t?? %d.aport led as %d.switchStatus","runLightSensor", "Port3", "On"],
				[" ", "???se t? ??e?st?? t?? ??µe?a? %d.port as %d.shutter","runShutter","Port1", "Press"],
				["-"],
				["h", "?ta? pat??e? t? ???µp? %m.buttonStatus","whenButtonPressed","pressed"],
				["R", "???µp? %m.buttonStatus","getButtonOnBoard","pressed"],
				["R", "a?s??t??a? f?t?? %d.laport","getLightSensor","light sensor on board"],
				["-"],
				["R", "a?s??t??a? ?p?????? %d.port distance","getUltrasonic","Port1"],
				["R", "a?s??t??a? ??aµµ?? %d.port","getLinefollower","Port1"],
				["R", "joystick %d.aport %d.Axis","getJoystick","Port3","X-Axis"],
				["R", "p??tes??µet?? %d.aport","getPotentiometer","Port3"],
				["R", "a?s??t??a? ???? %d.aport","getSoundSensor","Port3"],
				["R", "d?a??pt?? ????? %d.port %d.slot","getLimitswitch","Port1","Slot1"],
				["R", "?e?µ???as?a %d.port %d.slot °C","getTemperature","Port3","Slot1"],
				["R", "a?s??t??a? ????s?? %d.port","getPirmotion","Port2"],
				["-"],
				["R","t??e?e???st???? ?pe?????? %m.ircode pressed","getIrRemote","A"],
				["-"],
				[" ", "ste??e t? µ???µa t?? mBot %s","runIR", "hello"],
				["R", "t? µ???µa t?? mBot ??f???e","getIR"],
				["-"],
				["R", "?????µet??t??","getTimer", "0"],
				[" ", "epa?af??? ?????µet??t?","resetTimer", "0"]
			],
		pt: [
    ...
    var menus = {
		...
		el:{
			...
			port:["Port1","Port2","Port3","Port4"],
			aport:["Port3","Port4"],
			lport:["led st?? p?a??ta","Port1","Port2","Port3","Port4"],
			laport:["a?s??t??a? f?t?? st?? p?a??ta","Port3","Port4"],
			direction:["t???e µp??st?","t???e p?s?","st???e de???","st???e a??ste??"],
			points:[":"," "],
			note:["C2","D2","E2","F2","G2","A2","B2","C3","D3","E3","F3","G3","A3","B3","C4","D4","E4","F4","G4","A4","B4","C5","D5","E5","F5","G5","A5","B5","C6","D6","E6","F6","G6","A6","B6","C7","D7","E7","F7","G7","A7","B7","C8","D8"],
			beats:["??s?","??ta?t?","??d??","????????","??p??","??de????"],
			servovalue:[0,45,90,135,180],
  ...
  var descriptor = {
		blocks : blocks[lang],
		menus : menus[lang],
		url: 'https://o2br.github.io/makeblock.js'
	}
